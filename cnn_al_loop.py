...
for i in (5000,10000,20000):
    print(f"\n=== Active Learning with {int(i*100)}% of training data as unlabelled pool ===")
    al_pool_size  = int(i * len(X_train))  # start with 50% of training data as unlabelled pool
    ...
    threshold_df   = pd.DataFrame(best_thresholds).T
    threshold_path = os.path.join(OUTPUT_DIR, "cnn_baseline_best_thresholds_al.csv")
    threshold_df.to_csv(threshold_path)
    print(f"Saved thresholds to: {threshold_path}")
    #simplified run for later in case you want to skip on adding extra results for validating te learning paths with optimal thresholding
    threshold_df = pd.read_csv(threshold_path, index_col=0)
    ...
    results_path = os.path.join(OUTPUT_DIR, f"cnn_al_test_results_final_{i}.csv")
