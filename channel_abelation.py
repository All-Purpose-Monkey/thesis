...
OUTPUT_DIR = os.path.expanduser("~/thesis/results/cnn_mini/channel_abelation/")
...
X1, y1 = preprocess.mash_that("~/thesis/data/house_1/house1_binarized.csv", "~/thesis/data/house_1/stft_segments/2014/wk42/", path=True, max_dirs=32)  # only voltage channe
X1 = np.array(X1)
y1 = np.array(y1)
X2, y2 = preprocess.mash_that("~/thesis/data/house_1/house1_binarized.csv", "~/thesis/data/house_1/stft_segments/2014/voltage/", path=True)
X2 = np.array(X2)
y2 = np.array(y2)

print(f"X 1 shape: {len(X1)}, y shape: {len(y1)}")
print(f"Active samples per appliance: {y1.sum(axis=0)}")
print(f"X 2 shape: {len(X2)}, y shape: {len(y2)}")
...
for channel_name, X_raw, y_raw in channels:
    print(f"\n{'='*50}")
    print(f"Channel: {channel_name}")
    print(f"{'='*50}")

    run_dir = os.path.join(OUTPUT_DIR, channel_name)
    ...
    X_clean, y_clean = remove_nan_rows(X_raw, y_raw)
    ...
    print(f"\n========== FINAL TEST RESULTS ({channel_name.upper()}) ==========")
    print(results_df.to_string())

    results_df.to_csv(os.path.join(run_dir, f"test_results_{channel_name}.csv"))
...
summary_path = os.path.join(OUTPUT_DIR, "channel_ablation_summary.csv")
summary_df.to_csv(summary_path, index=False)
print(f"\nSaved channel ablation summary to: {summary_path}")
