...
# =========================================================
# KERNEL × STRIDE × POOL ABLATION LOOP
# =========================================================
...
for pool_size in POOL_SIZE:
    for stride in STRIDE:
        for kernel in KERNEL:
            ...
            print(f"\n{'='*60}")
            print(f"  pool: {pool_size}  |  stride: {stride}  |  kernel: {kernel}  |  tag: {tag}")
            print(f"  fixed — k_num: {K_NUM}  padding: {PADDINGS}  sym_pad: {SYM_PADS}")
...
summary_path = os.path.join(BASE_OUTPUT_DIR, "compression_ablation_summary_2.csv")
summary_df.to_csv(summary_path)

print(f"\n{'='*60}")
print("  COMPRESSION ABLATION COMPLETE")
print(f"{'='*60}")
