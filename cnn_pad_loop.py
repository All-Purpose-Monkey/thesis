...
BASE_OUTPUT_DIR = os.path.expanduser(
    "~/thesis/results/cnn_mini/parameter_ablation/padding/"
)
...
# =========================================================
# PADDING × SYM_PAD ABLATION LOOP
# =========================================================

all_summary_rows = []

for padding in PADDINGS:
    for sym_pad in SYM_PADS:

        tag        = f"pad_{padding}_sympad_{sym_pad}"
        OUTPUT_DIR = os.path.join(BASE_OUTPUT_DIR, tag)
        os.makedirs(OUTPUT_DIR, exist_ok=True)

        print(f"\n{'='*60}")
        print(f"  padding: {padding}  |  sym_pad: {sym_pad}  |  tag: {tag}")
        print(f"  fixed — k_num: {K_NUM}  pool: {POOL_SIZE}  stride: {STRIDE}  kernel: {KERNEL}")
        ...
        classifier = MultiHeadClassifier_mini(
            backbone,
            cfg["appliances"],
            k_num=K_NUM,          # keep head dims tied to backbone
        ).to(device)
        ...
        # save for reference — but read directly from best_thresholds dict (no CSV round-trip)
...
summary_path = os.path.join(BASE_OUTPUT_DIR, "padding_ablation_summary.csv")
summary_df.to_csv(summary_path)

print(f"\n{'='*60}")
print("  PADDING ABLATION COMPLETE")
print(f"{'='*60}")
